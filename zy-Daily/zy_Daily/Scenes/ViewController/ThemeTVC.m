...
@interface ThemeTVC () <NewsManagerDelegate>
...
- (void)viewDidLoad {
    ...
    // 注册
    [self.tableView registerNib:[UINib nibWithNibName:@"ImagesLabelTVCell" bundle:nil]  forCellReuseIdentifier:@"Cell3"];
    [NewsManager shareNewsManager].delegate = self;
    [[NewsManager shareNewsManager] getThemes];
    [self createAct];
}
...
-(void)reload{
    self.themes = [NewsManager shareNewsManager].theme8;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ImagesLabelTVCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Cell3" forIndexPath:indexPath];
    Theme *theme = self.themes[indexPath.row];
    cell.theme = theme;
    NSLog(@"%@", theme.thumbnail);
//    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:theme.thumbnail]];
    return cell;
}
